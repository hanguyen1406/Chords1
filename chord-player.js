...
let ChordPlayer = {
  ...
  handleNoteClick() {
    let $guitar = $('#guitar'); 
    let $note = $(this);
    let fretId = $note.parent().data('fret');//toa do theo chieu doc
    let stringId = $note.data('string');//toa do theo chieu ngang
  ...
  displayChords() {
    ...
    for(let i = 0; i < 1; i++) {
      ...
      for(let j = 0; j < chordInfo.notes.length; j++) {
        ...
        var s = chordInfo.notes[j].interval;
        var f = chordInfo.notes[j].note.getName(ChordPlayer.state.options);
        console.log("Note:", chordInfo.notes[j]);
      }
      // output += '</tr>';
    }
    // output += '</table>';
